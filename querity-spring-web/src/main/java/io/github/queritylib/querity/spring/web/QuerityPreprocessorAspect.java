...
public class QuerityPreprocessorAspect {
    ...
    private Object applyAnyWithPreprocessorAnnotation(Object arg, Parameter param) {
        return findWithProcessorAnnotation(param).map(annotation -> preprocessArg(arg, annotation)).orElse(arg);
    }
    ...
    private Query preprocessQuery(Query query, List<QueryPreprocessor> preprocessors) {
        Query queryWithPreprocessors = query.toBuilder().preprocessors(preprocessors).build();
    ...
    private List<QueryPreprocessor> getQueryPreprocessors(WithPreprocessor withPreprocessor) {
        return Arrays.stream(withPreprocessor.beanName()).map(beanName -> applicationContext.getBean(beanName, QueryPreprocessor.class)).toList();
    }
}
