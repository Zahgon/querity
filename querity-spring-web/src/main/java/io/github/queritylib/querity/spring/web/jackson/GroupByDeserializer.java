...
public class GroupByDeserializer extends StdDeserializer<GroupBy> {
    ...
    private static SimpleGroupBy parseSimpleGroupBy(JsonNode jsonNode) {
        ...
        List<String> propertyNames = StreamSupport.stream(Spliterators.spliteratorUnknownSize(propertyNamesNode.iterator(), Spliterator.ORDERED), false).map(JsonNode::asString).toList();
        return SimpleGroupBy.builder().propertyNames(propertyNames).build();
    }
    ...
    private static SimpleGroupBy parseGroupByWithExpressions(JsonNode jsonNode, DeserializationContext context) {
        ...
        List<PropertyExpression> expressions = StreamSupport.stream(Spliterators.spliteratorUnknownSize(expressionsNode.iterator(), Spliterator.ORDERED), false).map(node -> deserializeExpression(node, context)).toList();
        return SimpleGroupBy.builder().expressions(expressions).build();
    }
