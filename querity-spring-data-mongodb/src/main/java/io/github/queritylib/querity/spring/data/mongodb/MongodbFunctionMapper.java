...
public class MongodbFunctionMapper {
    ...
    private static Object mod(List<FunctionArgument> args) {
        return new Document("$mod", List.of(toExpressionOrLiteral(args.get(0)), toExpressionOrLiteral(args.get(1))));
    }

    // --- String functions ---
    private static Object concat(List<FunctionArgument> args) {
        List<Object> mongoArgs = args.stream().map(MongodbFunctionMapper::toExpressionOrLiteral).toList();
    ...
    private static Object length(List<FunctionArgument> args) {
        ...
        return new Document("$cond", java.util.Arrays.asList(new Document("$eq", java.util.Arrays.asList(new Document("$ifNull", java.util.Arrays.asList(input, null)), null)), null, new Document("$strLenCP", input)));
    }
    ...
    private static Object nullif(List<FunctionArgument> args) {
        ...
        return new Document("$cond", Arrays.asList(new Document("$eq", List.of(first, second)), null, first));
    }
