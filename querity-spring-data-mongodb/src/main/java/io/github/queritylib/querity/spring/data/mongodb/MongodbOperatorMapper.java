...
class MongodbOperatorMapper {
    ...
    private static Criteria getRegex(Criteria where, Object value, boolean negate) {
        return negate ? where.not().regex(value.toString(), "i") : where.regex(value.toString(), "i");
    }
    ...
    private static Document buildComparisonExpression(Object leftExpr, Object value, Operator operator) {
        String mongoOp = switch(operator) {
            ...
            case IS_NOT_NULL ->
                "$ne";
            default ->
                throw new UnsupportedOperationException("Operator " + operator + " is not supported with function expressions in MongoDB");
        };
        Object rightValue;
    ...
    private static Operator getNegatedOperator(Operator operator) {
        return switch(operator) {
            ...
            default ->
                throw new IllegalArgumentException("Operator " + operator + " does not support field-to-field comparison");
        };
    }
}
