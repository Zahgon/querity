...
public class SelectUtils {
    ...
    private static <T> Optional<Class<? extends T>> findClassWithNativeSelectWrapperConstructor(Set<Class<? extends T>> allClasses, Class<?> wrappedType) {
        return allClasses.stream().filter(clazz -> hasNativeSelectWrapperConstructorForType(clazz, wrappedType)).findAny();
    }

    private static boolean hasNativeSelectWrapperConstructorForType(Class<?> clazz, Class<?> wrappedType) {
        return Arrays.stream(clazz.getDeclaredConstructors()).filter(constructor -> constructor.getParameterCount() == 1).anyMatch(constructor -> {
            Type paramType = constructor.getGenericParameterTypes()[0];
            if (paramType instanceof ParameterizedType pt && NativeSelectWrapper.class.isAssignableFrom((Class<?>) pt.getRawType())) {
