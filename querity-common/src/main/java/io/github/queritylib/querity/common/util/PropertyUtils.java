...
public class PropertyUtils {
    ...
    private static <T> Class<?> getFieldType(Class<T> beanClass, String fieldName) {
        Field field = getAccessibleField(beanClass, fieldName).orElseThrow(() -> new IllegalArgumentException(String.format("Property %s not found in class %s", fieldName, beanClass.getSimpleName())));
        Class<?> fieldType = field.getType();
        if (isCollectionType(fieldType))
            fieldType = getGenericType(field, 0);
        return fieldType;
    }
